  ...
  CodeMirror.prototype = {
    ...
    getHelpers: function(pos, type) {
      ...
      if (mode.name) {
        ...
        for (let i = 0; i < mode.length; i++) {
          if (typeof mode[i][type] == "string") {
            // console.log('不知道具体情况，后续遇到再更改');
            if (help[mode[i][type]]) { found.push(help[mode[i][type]]); }
          } else if (mode[i][type]) {
            // console.log('不知道具体情况，后续遇到再更改');
            // for (var i = 0; i < mode[i][type].length; i++) {
            //   var val = help[mode[i][type][i]];
            //   if (val) { found.push(val); }
            // }
          } else if (mode[i].helperType && help[mode[i].helperType]) {
            found.push(help[mode[i].helperType]);
          } else if (help[mode[i].name]) {
            found.push(help[mode[i].name]);
          }
        }          
      }
