...
function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
  if (!text) return
  let displayText = builder.splitSpaces ? splitSpaces(text, builder.trailingSpace) : text // splitSpaces()方法，将text中的某些空格更改为NBSP
...
function insertLineContent(line, builder, styles) {
  ...
  const stylesList = [];
  for (let i = 0; i < styles.length; i++) { // 转换多个style的格式并存入stylesList
    const stylesDemo = []
    stylesDemo.push([styles[i][0], styles[i][1], styles[i][2]])
    for (let j = 1; j < styles[i].length; j += 2) {
      if (j + 2 === styles[i].length) {
        break
      }
      stylesDemo.push([styles[i][j], styles[i][j + 2], styles[i][j + 3]])
    }
    stylesList.push(stylesDemo)
  }
  let noSt = []
  let upSt = []
  for (let i = stylesList.length - 1; i > 0; i--) { // 合并stylesList中的style
    for (let j = 0; j < stylesList[i].length; j++) { // 获取当前行中没有style的部分
      if (stylesList[i][j][2] === null || stylesList[i][j][2] === undefined || stylesList[i][j][2] === 'variable') {
        noSt.push(stylesList[i][j])
      }
    }
    for (let j = 0; j < noSt.length; j++) { // 找到合并的部分
      if (noSt[j][2] !== null && noSt[j][2] !== undefined && noSt[j][2] !== 'variable') {
        continue
      }
      let startIndex = -1
      let endIndex = -1
      for (let k = 0; k < stylesList[i - 1].length; k++) {
        if (stylesList[i - 1][k][0] === noSt[j][0]) {
          startIndex = k
        }
        if (stylesList[i - 1][k][1] === noSt[j][1]) {
          endIndex = k
        }
      }
      if (startIndex !== -1 && endIndex !== -1) {
        upSt = upSt.concat(stylesList[i - 1].slice(startIndex, endIndex + 1))
      } else {
        upSt = upSt.concat(noSt.slice(j, j + 1))
      }
    }
    stylesList.splice(i - 1, 1)
  }
  for (let i = 0; i < upSt.length; i++) { // 完成stylesList的更新
    for (let j = stylesList[0].length - 1; j >= 0; j--) {
      if (stylesList[0][j][1] == upSt[i][0]) {
        if (j + 1 > stylesList[0].length) {
          stylesList[0].push(j + 1, 0, upSt[i])
        } else {
          stylesList[0].splice(j + 1, 0, upSt[i])
        }
      }
      if (stylesList[0][j][0] == upSt[i][0]) {
        stylesList[0].splice(j, 1)
      }
    }
    if (upSt[i][0] === 1) {
      stylesList[0].unshift(upSt[i])
    }
  }
  styles = []
  for (let i = 0; i < stylesList[0].length; i++) {
    if (i == 0) {
      for (let j = 0; j < stylesList[0][i].length; j++) {
        styles.push(stylesList[0][i][j])
      }
    } else {
      for (let j = 1; j < stylesList[0][i].length; j++) {
        styles.push(stylesList[0][i][j])
      }        
    }
  }
  if (!spans) {
    for (let i = 1; i < styles.length; i+=2)
      ...
      builder.addToken(builder, allText.slice(at, at = styles[i]), interpretTokenStyle(styles[i+1], builder.cm.options)) // interpretTokenStyle()方法将mode返回的样式转换为CSS样式
    return
  }
