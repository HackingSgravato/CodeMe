...
(function (global, factory) {
  ...
  function innerMode(mode, state, type) {
    var info;
    if (type) {
  ...
  function highlightLine(cm, line, context, forceToEnd) {
    ...
    // 计算样式的基数组
    for (var i = 0; i < cm.doc.mode.length; i++) {
      st = [cm.state.modeGen];
  ...
  function getLineStyles(cm, line, updateFrontier) {
    ...
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      ...
      var stateDemo = context.save(!resetState);
      line.stateAfter = deep(stateDemo); // 与mode有关
      for (var i$1 = 0; i$1 < cm.display.view.length; i$1++) {
        if (cm.display.view[i$1].line.text === line.text && i$1 > 0) {
          cm.display.view[i$1 - 1].line.stateAfter = deep(oldDemo);
        }
      }
  ...
  function getContextBefore(cm, n, precise) { // 获取context，与mode有关
  ...
  function takeToken(cm, pos, precise, asArray, type) {
  ...
  // 呈现行的文本的DOM表示。还构建了一个“行图”，它指向表示特定文本延伸的DOM节点，
  // 并由度量代码使用。返回的对象包含DOM节点、此映射以及有关由模式设置的行范围样式的信息。
  function buildLineContent(cm, lineView) {
    // The padding-right forces the element to have a 'border', which
    // is needed on Webkit to be able to get line-level bounding
    // rectangles for it (in measureChar).
    // 右边的填充强制元素有一个“border”，这在Webkit上是必需的，以便能够为它获得行级边界矩形（在measureChar中）。
    var content = eltP("span", null, null, webkit ? "padding-right: .1px" : null);
    var builder = {pre: eltP("pre", [content], "CodeMirror-line"),
                   content: content,
                   col: 0,
                   pos: 0,
                   cm: cm,
    ...
    for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
      ...
      // 有选择地将一些黑客连接到token呈现算法中，以处理浏览器的怪癖。
      if (hasBadBidiRects(cm.display.measure) && (order = getOrder(line, cm.doc.direction))) {
        builder.addToken = buildTokenBadBidi(builder.addToken, order); // 解决从右到左的文本拉伸报告的无意义维度
      }
      builder.map = [];
      var allowFrontierUpdate = lineView != cm.display.externalMeasured && lineNo(line);

      // insertLineContent()方法，输出多个跨距以组成一行，同时考虑突出显示和标记的文本
      insertLineContent(line, builder, getLineStyles(cm, line, allowFrontierUpdate)); // getLineStyles()方法获取当前行代码的样式
  ...
  // 为单个标记构建DOM表示，并将其添加到行映射中。注意分别渲染特殊字符。
  function buildToken(builder, text, style, startStyle, endStyle, css, attributes) {
    if (!text) { return }
    var displayText = builder.splitSpaces ? splitSpaces(text, builder.trailingSpace) : text; // splitSpaces()方法，将text中的某些空格更改为NBSP
  ...
  // 输出多个跨距以组成一行，同时考虑突出显示和标记的文本。
  function insertLineContent(line, builder, styles) {
    var spans = line.markedSpans, allText = line.text, at = 0;
    var stylesList = [];
    for (var i$1 = 0; i$1 < styles.length; i$1++) { // 转换多个style的格式并存入stylesList
      var stylesDemo = [];
      stylesDemo.push([styles[i$1][0], styles[i$1][1], styles[i$1][2]]);
      for (var j = 1; j < styles[i$1].length; j += 2) {
        if (j + 2 === styles[i$1].length) {
          break
        }
        stylesDemo.push([styles[i$1][j], styles[i$1][j + 2], styles[i$1][j + 3]]);
      }
      stylesList.push(stylesDemo);
    }
    var noSt = [];
    var upSt = [];
    for (var i$2 = stylesList.length - 1; i$2 > 0; i$2--) { // 合并stylesList中的style
      for (var j$1 = 0; j$1 < stylesList[i$2].length; j$1++) { // 获取当前行中没有style的部分
        if (stylesList[i$2][j$1][2] === null || stylesList[i$2][j$1][2] === undefined || stylesList[i$2][j$1][2] === 'variable') {
          noSt.push(stylesList[i$2][j$1]);
        }
      }
      for (var j$2 = 0; j$2 < noSt.length; j$2++) { // 找到合并的部分
        if (noSt[j$2][2] !== null && noSt[j$2][2] !== undefined && noSt[j$2][2] !== 'variable') {
          continue
        }
        var startIndex = -1;
        var endIndex = -1;
        for (var k = 0; k < stylesList[i$2 - 1].length; k++) {
          if (stylesList[i$2 - 1][k][0] === noSt[j$2][0]) {
            startIndex = k;
          }
          if (stylesList[i$2 - 1][k][1] === noSt[j$2][1]) {
            endIndex = k;
          }
        }
        if (startIndex !== -1 && endIndex !== -1) {
          upSt = upSt.concat(stylesList[i$2 - 1].slice(startIndex, endIndex + 1));
        } else {
          upSt = upSt.concat(noSt.slice(j$2, j$2 + 1));
        }
      }
      stylesList.splice(i$2 - 1, 1);
    }
    for (var i$3 = 0; i$3 < upSt.length; i$3++) { // 完成stylesList的更新
      for (var j$3 = stylesList[0].length - 1; j$3 >= 0; j$3--) {
        if (stylesList[0][j$3][1] == upSt[i$3][0]) {
          if (j$3 + 1 > stylesList[0].length) {
            stylesList[0].push(j$3 + 1, 0, upSt[i$3]);
          } else {
            stylesList[0].splice(j$3 + 1, 0, upSt[i$3]);
          }
        }
        if (stylesList[0][j$3][0] == upSt[i$3][0]) {
          stylesList[0].splice(j$3, 1);
        }
      }
      if (upSt[i$3][0] === 1) {
        stylesList[0].unshift(upSt[i$3]);
      }
    }
    styles = [];
    for (var i$4 = 0; i$4 < stylesList[0].length; i$4++) {
      if (i$4 == 0) {
        for (var j$4 = 0; j$4 < stylesList[0][i$4].length; j$4++) {
          styles.push(stylesList[0][i$4][j$4]);
        }
      } else {
        for (var j$5 = 1; j$5 < stylesList[0][i$4].length; j$5++) {
          styles.push(stylesList[0][i$4][j$5]);
        }        
      }
    }
    if (!spans) {
      for (var i$5 = 1; i$5 < styles.length; i$5+=2)
        // builder.addToken为buildToken()方法，为单个标记构建DOM表示，并将其添加到行映射中
        { builder.addToken(builder, allText.slice(at, at = styles[i$5]), interpretTokenStyle(styles[i$5+1], builder.cm.options)); } // interpretTokenStyle()方法将mode返回的样式转换为CSS样式
      return
    }
    ...
    for (;;) {
      if (nextChange == pos) { // Update current marker set
        ...
        for (var j$6 = 0; j$6 < spans.length; ++j$6) {
          var sp = spans[j$6], m = sp.marker;
        ...
        if (endStyles) { for (var j$7 = 0; j$7 < endStyles.length; j$7 += 2)
          { if (endStyles[j$7 + 1] == nextChange) { spanEndStyle += " " + endStyles[j$7]; } } }

        if (!collapsed || collapsed.from == pos) { for (var j$8 = 0; j$8 < foundBookmarks.length; ++j$8)
          { buildCollapsedSpan(builder, 0, foundBookmarks[j$8]); } }
  ...
  // 包装buildLineContent，如果可能，它将重用display.externalMeasured的结构
  function getLineContent(cm, lineView) {
    ...
    return buildLineContent(cm, lineView) // 呈现行的文本的DOM表示
  }
  ...
  function buildLineElement(cm, lineView, lineN, dims) {
    var built = getLineContent(cm, lineView); // 包装buildLineContent，如果可能，它将重用display.externalMeasured的结构
    lineView.text = lineView.node = built.pre;
    if (built.bgClass) { lineView.bgClass = built.bgClass; }
    if (built.textClass) { lineView.textClass = built.textClass; }

    updateLineClasses(cm, lineView); // 通过修改每行的className来控制样式
    updateLineGutter(cm, lineView, lineN, dims); // 通过更新文本左侧行号的className来控制样式
    insertLineWidgets(cm, lineView, dims); // 插入行的小部件
    return lineView.node
  }
  ...
  //公开可见的API。注意methodOp（f）表示'将f包装在操作中，对其'this'参数执行。

  // This is not the complete set of editor methods. Most of the
  // methods defined on the Doc type are also injected into
  // CodeMirror.prototype, for backwards compatibility and
  // convenience.
  //这不是完整的编辑器方法集。大部分在Doc类型上定义的方法也被注入到代码镜像.原型，用于向后兼容性和方便。
  ...
  function addEditorMethods(CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      // 获取给定字符的解析器标记。对于想要检查mode状态的黑客来说很有用。
      getTokenAt: function(pos, precise, type) {
        return takeToken(this, pos, precise, undefined, type)
      },
      ...
      getModeAt: function(pos) {
        ...
        for (var i = 0; i < mode.length; i++) {
          if (mode[i].innerMode) {
            mode[i] = CodeMirror.innerMode(mode[i], this.getTokenAt(pos).state).mode;
          }
        }
        return mode
      },
      ...
      getHelpers: function(pos, type) {
        ...
        var help = helpers[type], mode = this.getModeAt(pos); // getModeAt()方法获取当前模式
        if (mode.name) {
          ...
          for (var i$2 = 0; i$2 < mode.length; i$2++) {
            if (typeof mode[i$2][type] == "string") {
              // console.log('不知道具体情况，后续遇到再更改');
              if (help[mode[i$2][type]]) { found.push(help[mode[i$2][type]]); }
            } else if (mode[i$2][type]) ; else if (mode[i$2].helperType && help[mode[i$2].helperType]) {
              found.push(help[mode[i$2].helperType]);
            } else if (help[mode[i$2].name]) {
              found.push(help[mode[i$2].name]);
            }
          }          
        }
